...
private:
    ...
    double center_sd_ = 0.0;
    std::uint32_t read_length_ = 0;
    std::uint32_t maximum_ambiguous_count_ = 0;
    std::uint32_t total_weight_ = 0;
    bool paired_end_ = false;
    bool variable_insert_ = false;
    SamplingMode sampling_mode_ = SamplingMode::uniform;
};

// Per-contig target sampler. Mean-insert geometry defines the eligible target
// domain. Each output then selects one target and deterministically retries
// configured insert-length and center-displacement draws for that same target
// until it obtains a valid fragment or exhausts the fixed attempt cap.
