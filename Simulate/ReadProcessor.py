...
class ReadProcessor:
    '''read processor'''
    def __init__(self, pos_map: np.ndarray = None, meth_arr: np.ndarray = None,
                 var_profile: Dict = None, experiment: SetExperiment = None) -> Dict:
    ...
    def process_read_pair3(self, read_data):
        """
    ...
    def fetch_meth_val(self, pos_arr, arr_idx, var_type):
        ...
        if var_type == 1:           # SNP
            ...
            pos_idx = self.pos_map[pos_arr]
            if 4294967295 in pos_idx:
                return None
            meth_val= list(self.meth_arr[pos_idx, arr_idx])
    ...
    def fetch_meth_state(self, read_meth):
        ...
        if self.experiment.site_dependency: #TODO:
    ...
    def process_error(self, read_pair):
        '''save the intermediate results for later check'''
        file_id = read_pair[0]["read_id"].replace(":", "_")
        with open(f'{self.experiment.tmp_dir}/{file_id[1:]}.pkl', 'wb') as FILE:
            pickle.dump(read_pair, FILE)
