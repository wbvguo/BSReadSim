...
RUN_MODES = ("production", "debug")
WGBS_GC_PROFILE_FORMAT = "tsv"
WGBS_GC_PROFILE_VERSION = "wgbs-gc-target-v1"
    ...
    *,
    ...
    if not isinstance(mode, str) or mode not in RUN_MODES:
        raise ConfigValidationError(
            "run mode must be one of {}".format(", ".join(RUN_MODES))
        )
    ...
    # Truth is a runtime mode, not a user configuration knob.
    normalized["output"]["truth"] = "full" if mode == "debug" else "none"
...
def _load_schema() -> Dict[str, Any]:
    schema_path = Path(__file__).with_name(RUN_CONFIG_SCHEMA_FILENAME)
    try:
...
def _parse_seed(seed: Any) -> Optional[int]:
    if seed is None:
        return None
...
def _validate_cross_field_rules(config: Mapping[str, Any]) -> None:
    ...
    if (
        technology == "RRBS"
        and has_rrbs_candidates
        and ("vcf" in inputs or mutation["rate"] > 0)
        and "seed" not in config
    ):
        raise ConfigValidationError(
            "$.seed: RRBS candidate BED with variants requires an explicit seed"
        )
