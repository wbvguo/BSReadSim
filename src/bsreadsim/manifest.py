...
from .bam import TRUTH_BAM_CONTRACT, TRUTH_BAM_MAPQ
from .config import RUN_CONFIG_SCHEMA_VERSION
from .output import OutputFileSummary, OutputSummary
from .preparation import FileDigest, PreparedRun
...
from .read_names import READ_NAME_CONTRACT
from .rng import RNG_CONTRACT, STAGE_NAMES


MANIFEST_SCHEMA_VERSION = "1.0"
ANNOTATION_FORMAT = "bsreadsim-truth-jsonl"
ANNOTATION_FORMAT_VERSION = "1.2"
...
def build_complete_manifest(
    ...
    document = {
        ...
        "versions": {
            "annotation": ANNOTATION_FORMAT_VERSION,
    ...
    if config.normalized["output"]["truth_bam"]:
        document["truth_alignment"] = {
            "contract": TRUTH_BAM_CONTRACT,
            "format": "BAM",
            "mapq": TRUTH_BAM_MAPQ,
        ...
        document["versions"]["truth_bam"] = TRUTH_BAM_CONTRACT
...
def validate_header_projection(prepared: PreparedRun, header: Header) -> None:
    ...
    expected_truth_columns = (
        "full"
        if normalized_output["truth"] == "full" or normalized_output["truth_bam"]
        else "none"
    )
    if expected_truth_columns not in ("none", "full"):
        raise ManifestError("expected Truth columns must be none or full")
    ...
    expected_truth = (
        TruthMode.FULL
        if expected_truth_columns == "full"
        else TruthMode.NONE
    )
    if header.truth_columns is not expected_truth:
        raise ManifestError("core Truth columns disagree with the requested mode")
...
def _validate_counts(
    ...
    truth_mode = config["output"]["truth"]
    if truth_mode == "full":
        expected_roles.add("truth")
    elif truth_mode != "none":
        raise ManifestError("output truth mode is unsupported")
    if config["output"]["truth_bam"]:
        expected_roles.add("truth_bam")
