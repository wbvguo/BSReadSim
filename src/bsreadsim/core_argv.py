...
def build_core_argv(
    ...
    *,
    ...
    if truth_columns not in ("none", "full"):
        raise CoreArgvError("truth_columns must be none or full")
    if (
    ...
    for input_name in ("vcf", "cgmap", "bed_methyl", "asm", "asm_bed"):
...
def _expected_roles(
    ...
    expected = {}  # type: Dict[str, Tuple[Path, Optional[str]]]

    def add(role: str, path_value: Any, declared_sha256: Optional[str] = None) -> None:
        if role in expected:
...
__all__ = ["CoreArgvError", "build_core_argv"]
