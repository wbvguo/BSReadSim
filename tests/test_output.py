...
from bsreadsim.output import (  # noqa: E402
    ...
    OutputTransaction,
    TruthBamConfig,
    _canonical_truth_json,
    _canonical_truth_json_bytes,
    _canonical_truth_json_python,
    _format_fragment_records,
...
from tests.test_postprocess import make_fragment  # noqa: E402
from bsreadsim.model import VariantEvent, VariantKind  # noqa: E402
...
class OutputTransactionTests(unittest.TestCase):
    ...
    def truth_bam_config(self, program: str) -> TruthBamConfig:
        return TruthBamConfig(
    ...
    def test_truth_encoder_remains_canonical_sorted_json(self) -> None:
        values = (
            processed(),
            processed(paired_end=True),
            processed_deletion(),
            replace(processed(), contig_name='chr\"雪\\line\n'),
        )
        for value in values:
            with self.subTest(value=value):
                encoded = _canonical_truth_json(value)
                self.assertEqual(encoded, _canonical_truth_json_python(value))
                self.assertEqual(
                    _canonical_truth_json_bytes(value),
                    encoded.encode("utf-8"),
                )
                self.assertEqual(
                    _canonical_truth_json_bytes(value, newline=True),
    ...
    def test_compact_worker_annotations_preserve_exact_truth_bytes(self) -> None:
        for paired_end in (False, True):
            with self.subTest(paired_end=paired_end):
                ...
                self.assertEqual(
                    _canonical_truth_json_python(compact),
                    _canonical_truth_json_python(regular),
                )
                self.assertEqual(
                    _format_fragment_records_trusted(
                        compact,
    ...
    def test_single_end_files_are_invisible_until_commit(self) -> None:
        transaction = OutputTransaction(self.config())
        ...
        truth = self.directory / "sample.truth.jsonl"
        self.assertEqual(
            fastq.read_text(encoding="utf-8"),
            "@chr1:101-108:0/1\nACTGT\n+\n?????\n",
        )
        truth_value = json.loads(truth.read_text(encoding="utf-8"))
        self.assertEqual(truth_value["fragment_ordinal"], 0)
        self.assertEqual(truth_value["mates"][0]["reference_start"], 100)
        self.assertEqual(truth_value["variant_events"], [])
        self.assertEqual(
            truth_value["site_states"][0],
            {
                "allele": "SHARED",
                "context": "CG_C",
                "methylated": True,
    ...
    def test_fastq_only_omits_truth_artifact_and_accepts_no_annotations(self) -> None:
        config = self.config(paired_end=True, truth="none")
        with OutputTransaction(config) as transaction:
        ...
        self.assertFalse((self.directory / "sample.truth.jsonl").exists())
    ...
    def test_fastq_only_formatted_batch_is_supported(self) -> None:
        ...
        with OutputTransaction(config) as transaction:
            transaction.write_formatted_batch(
                0,
                tuple(
                    (len(read1), len(read2), 0)
                    for read1, read2, _ in formatted
                ),
    ...
    def test_deletion_event_is_preserved_in_truth_output(self) -> None:
        with OutputTransaction(self.config()) as transaction:
            transaction.write_fragment(processed_deletion())
            summary = transaction.finalize()
            transaction.commit(manifest_json(summary))

        truth_value = json.loads(
            (self.directory / "sample.truth.jsonl").read_text(encoding="utf-8")
        )
        self.assertEqual(
            truth_value["variant_events"],
            [
                {
                    "alt_bases": "",
                    "event_id": 7,
    ...
    def test_paired_mates_are_published_together(self) -> None:
        with OutputTransaction(self.config(paired_end=True)) as transaction:
    ...
    def test_truth_bam_is_streamed_and_counted_per_mate(self) -> None:
        program = (
        ...
        config = self.config(
            paired_end=True,
            truth="none",
            truth_bam=self.truth_bam_config(program),
        )
        with OutputTransaction(config) as transaction:
            transaction.write_fragment(processed(paired_end=True))
            summary = transaction.finalize()
        ...
        bam = self.directory / "sample.truth.bam"
        self.assertTrue(bam.read_bytes().startswith(b"fake-bam\n@HD\tVN:1.6"))
        self.assertEqual(summary.file_for_role("truth_bam").record_count, 2)
        self.assertEqual(
            tuple(item.role for item in summary.files),
            ("read1", "read2", "truth_bam"),
        )
        self.assertFalse((self.directory / "sample.truth.jsonl").exists())

    def test_truth_bam_writer_failure_aborts_every_output(self) -> None:
        ...
        self.assertFalse((self.directory / "sample.R1.fastq").exists())
        self.assertFalse((self.directory / "sample.truth.bam").exists())
        ...
        for level in (1, 6):
            ...
            with OutputTransaction(config) as transaction:
                for ordinal in range(100):
                    transaction.write_fragment(
                        processed(ordinal=ordinal, include_truth=False)
                    )
                summary = transaction.finalize()
                transaction.commit(manifest_json(summary))
    ...
    def test_formatted_batch_is_byte_identical_to_fragment_writes(self) -> None:
        ...
        formatted = tuple(
            _format_fragment_records(fragment, paired_end=True)
            for fragment in fragments
        )
        lengths = tuple(
            (len(read1), len(read2), len(truth))
        ...
        with OutputTransaction(
            ...
            transaction.write_formatted_batch(
                ...
                memoryview(b"".join(item[2] for item in formatted)),
            )
    ...
    def test_invalid_policy_is_rejected_before_staging(self) -> None:
        ...
        with self.assertRaisesRegex(OutputError, "truth must"):
            self.config(truth="compact")
    ...
    def test_manifest_is_required_and_must_be_canonical_complete_json(self) -> None:
    ...
    def test_manifest_must_identify_the_exact_staged_bytes(self) -> None:
        transaction = OutputTransaction(self.config())
        transaction.write_fragment(processed())
        summary = transaction.finalize()
