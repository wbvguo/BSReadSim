"""Contract tests for SAM formatting behind truth-aligned BAM output."""
...
from bsreadsim.bam import (  # noqa: E402
    TruthBamError,
...
from bsreadsim.protocol import Contig  # noqa: E402
from tests.test_postprocess import make_fragment  # noqa: E402
from tests.test_protocol import make_header  # noqa: E402


RUN_ID = "00000000-0000-4000-8000-000000000002"


def annotation(offset: int, position: int) -> BaseAnnotation:
    return BaseAnnotation(
        read_offset=offset,
        reference_pos=position,
        variant_event_id=0xFFFFFFFF,
        site_index=None,
        methylated=None,
        oriented_base=0,
...
def fragment_for(
    ...
    *,
    ...
    mate = ProcessedMate(
        ...
        conversion_mode=ConversionMode.C_TO_T,
    ...
    return ProcessedFragment(
        ...
        reference_end=reference_end,
        haplotype=1,
        fragment_conversion_mode=ConversionMode.C_TO_T,
        variant_events=(),
        site_states=(),
        mates=(mate,),
    )
...
class TruthBamFormattingTests(unittest.TestCase):
    def test_header_declares_unsorted_sam_16_dictionary_and_provenance(self) -> None:
        value = build_truth_sam_header(
            make_header(truth=True),
            sample_name="sample",
    ...
    def test_indel_projection_forms_query_complete_cigar(self) -> None:
        record = format_truth_sam_fragment(
        ...
        self.assertEqual(value[11:], ["RG:Z:" + RUN_ID, "PG:Z:bsreadsim"])
    ...
    def test_paired_records_have_standard_flags_mate_fields_and_tlen(self) -> None:
        ...
        first, second = tuple(
            fields(record)
            for record in format_truth_sam_fragment(
                processed,
                paired_end=True,
                read_group_id="run",
                contig_length=1000,
        ...
        self.assertEqual(first[-1], "MC:Z:5M")
        self.assertEqual(second[-1], "MC:Z:5M")
    ...
    def test_reference_dictionary_rejects_names_other_tools_cannot_parse(self) -> None:
        ...
        with self.assertRaisesRegex(TruthBamError, "reference name"):
            build_truth_sam_header(
                header,
                sample_name="sample",
                program_version="1.2.3",
    ...
    def test_query_name_enforces_sam_character_and_length_limits(self) -> None:
        ...
        for contig_name in invalid_names:
            with self.subTest(contig_name=contig_name[:12]):
                ...
                with self.assertRaisesRegex(TruthBamError, "query name"):
                    format_truth_sam_fragment(
                        fragment,
                        paired_end=False,
                        read_group_id=RUN_ID,
