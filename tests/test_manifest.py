...
from bsreadsim.output import OutputConfig, OutputTransaction  # noqa: E402
...
from bsreadsim.read_names import READ_NAME_CONTRACT  # noqa: E402
...
class ManifestTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.prepared = prepare_run(
            normalize_run_config(document, self.directory, mode="debug")
        )
        self.header = Header(
            run_id=RUN_ID,
    ...
    def staged_outputs(self):
        transaction = OutputTransaction(self.output_config)
    ...
    def test_manifest_cross_checks_and_commits_the_run(self) -> None:
    ...
    def test_fastq_only_manifest_accepts_exactly_the_read_roles(self) -> None:
        ...
        object.__setattr__(header, "truth_columns", TruthMode.NONE)
        ...
        transaction = OutputTransaction(output_config)
        transaction.write_fragment(
            process_fragment(
                make_fragment(paired_end=False),
                "chr1",
        ...
        transaction.commit(manifest.canonical_json)
        self.assertFalse(
            (self.directory / "fastq-output/sample.truth.jsonl").exists()
        )

    def test_manifest_digest_detects_any_stable_field_change(self) -> None:
        transaction, outputs = self.staged_outputs()
    ...
    def test_header_projection_checks_execution_shape_and_truth_mode(self) -> None:
        header = Header(
            run_id=RUN_ID,
            core_version=__version__,
            config_schema_version="1.0",
        ...
        failures = (
            ...
            (replace(header, truth_columns=TruthMode.NONE), "Truth"),
            (
    ...
    def test_truth_bam_requires_full_projection_without_json_truth(self) -> None:
        document = base_config()
        document["reference"] = "reference.fa"
        document["seed"] = "7"
        document["output"] = {
        ...
        with self.assertRaisesRegex(ManifestError, "Truth"):
            validate_header_projection(
                prepared,
                replace(header, truth_columns=TruthMode.NONE),
            )
